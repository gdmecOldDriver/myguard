...
import edu.gdmec.s07150815.myguard.SplashActivity;
...
public class ContactInfoParser {
    public static List<ContactInfo> getSystemContact(Context context){
        ...
        while (cursor.moveToNext()){
            ...
            if (id!=null){
                ...
                info.id = id;
                //根据联系人id，查找data表，把这个id的数据取出来
                //系统api 查询data表的时候，不是真正的查询data表，而是查询的data表的视图
                Cursor dataCursor = resolver.query(darauri,new String[]{"data1","mimetype"},"raw_contact_id",new String[]{id},null);
    ...
    public static List<ContactInfo> getSimContacts(Context context){
        Uri uri = Uri.parse("contact://icc/adn");
        List<ContactInfo> infos =new ArrayList<ContactInfo>();
