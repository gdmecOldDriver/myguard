...
public class NumBelongtoDao {
    ...
    public static String getLocation(String phonenumber){
        ...
        //长度是11位
        //^1[134578]\d{9}$
        SQLiteDatabase db = SQLiteDatabase.openDatabase("/data/data/edu.gdmec.s07150815.myguard/files/address.db",null
                ...
                switch (phonenumber.length()){//判断电话号码的长度
                    ...
                    default:
                        if(location.length()>=9&& location.startsWith("0")){
                            ...
                            cursor.close();
                            cursor = db.rawQuery("select location from data2 where area = ?",new String []{
